...
export const createVideoAction = async (data:VideoProp) => {
...
export const deleteVideoUrl = async (id: string) => {
  const { getUser, getAccessToken } = getKindeServerSession();
  const user = await getUser();
  const token = await getAccessToken();
  ...
  try {
    const response = await axios.delete(`${STRAPI_API_URLBASE}/api/upload/files/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
